...
$query_count = mysqli_query($conn, "SELECT id_racikan FROM racikan");
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php
                        $query = mysqli_query($conn, "SELECT * FROM racikan ORDER BY id_racikan DESC");
                        if(mysqli_num_rows($query) > 0) {
                            while($data = mysqli_fetch_array($query)) :
                                $id_r = $data['id_racikan'];
                                // Ambil komposisi bahan dalam satu string
                                $q_bahan = mysqli_query($conn, "SELECT m.nama_obat FROM racikan_detail rd JOIN medicines m ON rd.id_obat = m.id WHERE rd.id_racikan = '$id_r'");
                                $bahan_list = [];
