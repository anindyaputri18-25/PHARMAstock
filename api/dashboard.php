...
// Logika pengambilan data statistik untuk Dashboard
$stok_aman = 0;
$stok_menipis = 0;
$stok_habis = 0;
...
if ($role != 'Pending') {
    // Stok Aman: > 15
    $q_aman = mysqli_query($conn, "SELECT COUNT(*) as total FROM medicines WHERE jumlah > 15");
    $stok_aman = mysqli_fetch_assoc($q_aman)['total'] ?? 0;

    // Stok Menipis: 1 - 15
    $q_menipis = mysqli_query($conn, "SELECT COUNT(*) as total FROM medicines WHERE jumlah > 0 AND jumlah <= 15");
    $stok_menipis = mysqli_fetch_assoc($q_menipis)['total'] ?? 0;

    // Obat Kosong: <= 0
    $q_habis = mysqli_query($conn, "SELECT COUNT(*) as total FROM medicines WHERE jumlah <= 0");
    $stok_habis = mysqli_fetch_assoc($q_habis)['total'] ?? 0;
}
?>
    ...
    <main class="flex-1 p-6 md:p-10 lg:p-12 max-w-[1600px] mx-auto w-full">
        <header class="flex justify-between items-center mb-10 print:hidden">
            ...
            <div class="flex items-center gap-3 bg-white p-1.5 rounded-full border border-slate-100 smooth-shadow">
                <div class="flex flex-col items-end px-3">
                    ...
                    <p class="text-xs font-black text-slate-800"><?php echo htmlspecialchars($user); ?></p>
                </div>
        ...
        <?php if ($role == 'Pending') : ?>
            <div class="glass-card p-12 rounded-[3rem] text-center max-w-xl mx-auto mt-10">
                ...
                <p class="text-slate-500 text-sm font-medium leading-relaxed italic">Halo <?php echo htmlspecialchars($user); ?>, Admin sedang meninjau akunmu.</p>
            </div>
        <?php else : ?>
